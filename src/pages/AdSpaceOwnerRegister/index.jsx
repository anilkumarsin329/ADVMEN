...
const AdSpaceOwnerRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) navigate('/ad-space/owner/dashboard')
    } catch (err) {
  ...
  return (
    <PageTransition>
      <SEOHead title="Space Owner Registration — ADVMEN" />
      <div className="min-h-screen bg-black pt-32 pb-24 flex items-center justify-center">
        <div className="w-full max-w-md p-8 rounded-2xl bg-white/5 border border-[rgba(255,107,0,0.15)] backdrop-blur-md">
          ...
          <div className="text-center mb-8">
            <h1 className="text-2xl font-display font-bold text-white mb-2">Become a Space Owner</h1>
            <p className="text-sm text-slate-400">List your properties and start earning.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-mono uppercase text-slate-400 mb-2">Full Name</label>
          ...
          <div className="mt-6 text-center text-sm text-slate-400">
            Already have an account? <Link to="/ad-space/owner/login" className="text-orange-500 hover:underline">Login here</Link>
          </div>
        </div>
      </div>
    </PageTransition>
...
export default AdSpaceOwnerRegister
