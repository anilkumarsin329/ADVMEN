...
const AdSpaceAdvertiserLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        if (res.data.role === 'advertiser') navigate('/ad-space/advertiser/dashboard')
        else setError('Please login through owner portal')
      }
    } catch (err) {
  ...
  return (
    <PageTransition>
      <SEOHead title="Advertiser Login — ADVMEN" />
      <div className="min-h-screen bg-black pt-32 pb-24 flex items-center justify-center">
        <div className="w-full max-w-md p-8 rounded-2xl bg-white/5 border border-[rgba(255,107,0,0.15)] backdrop-blur-md">
          ...
          <div className="text-center mb-8">
            <h1 className="text-2xl font-display font-bold text-white mb-2">Advertiser Login</h1>
          ...
          <div className="mt-6 text-center text-sm text-slate-400">
            Don't have an account? <Link to="/ad-space/advertiser/register" className="text-orange-500 hover:underline">Register here</Link>
          </div>
        </div>
      </div>
    </PageTransition>
...
export default AdSpaceAdvertiserLogin
