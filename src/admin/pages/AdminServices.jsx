...
const AdminServices = () => {
  ...
  const fetchservicesData = async () => {
    ...
    try {
      ...
      console.warn('Backend services API not reachable, setting empty list:', err)
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  const uniqueSlugs = ['All', ...new Set(items.map(item => item.slug))]
  ...
  const openFormModal = (item = null) => {
    ...
    if (item) {
      ...
      const isDefaultOrExisting = dropdownCats.includes(item.slug)
      if (isDefaultOrExisting) {
        setIsCustomCategory(false)
        setCustomCategoryValue('')
      } else {
        setIsCustomCategory(true)
        setCustomCategoryValue(item.slug)
      }
    } else {
      setCurrentItem(null)
      setFormValues({
  ...
  const validateForm = () => {
    ...
    if (!formValues.tagline.trim()) errors.tagline = 'Tagline is required'
    ...
    try {
      if (currentItem) {
        ...
        if (response.ok) {
          ...
          showToast('services updated (local offline mode).')
        }
      } else {
        // Create Mode
        ...
        if (response.ok) {
          ...
          showToast('Service created (local offline mode).')
        }
      }
      setIsModalOpen(false)
    } catch (err) {
      console.warn('services API connection failed, updating state locally:', err)
      if (currentItem) {
        setItems(items.map(item => (item.id === currentItem.id ? { ...item, ...payload } : item)))
        showToast('Service updated (local offline mode).')
      } else {
        const mockNewItem = {
          ...payload,
          id: String(Date.now()),
  ...
  const handleToggleStatus = async (item) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(`Service ${data.item.isActive ? 'enabled' : 'disabled'} successfully.`)
      } else {
        showToast(`Item availability status toggled (local offline mode).`)
      }
    } catch (err) {
      console.warn('services toggle API failed, fallback to local state:', err)
      showToast(`Item availability status toggled (local offline mode).`)
    }
  }
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setItems(items.filter(item => item.id !== deleteItemTarget.id))
        showToast('Service deleted (local offline mode).', 'error')
      }
    } catch (err) {
      console.warn('services API connection failed, deleting locally:', err)
      setItems(items.filter(item => item.id !== deleteItemTarget.id))
      showToast('Service deleted (local offline mode).', 'error')
    } finally {
  ...
  const filteredItems = items.filter(item => {
    ...
    const matchesCategory = selectedSlug === 'All' || item.slug === selectedSlug
    return matchesSearch && matchesCategory
  })

  return (
    <div className="admin-services-container w-full space-y-8 pb-10" style={{ color: 'var(--admin-text-primary)' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .admin-services-container label {
          color: #555555 !important;
        }
        .admin-services-container span.badge-gray {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <span className="font-mono text-[9px] uppercase tracking-wider font-bold text-green-500">
              Active Services
            </span>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm" style={{ boxShadow: 'var(--admin-shadow-neu-inset)', color: 'var(--admin-text-secondary)' }}>
              <FiCheck />
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <span className="font-mono text-[9px] uppercase tracking-wider font-bold text-amber-500">
              Inactive Services
            </span>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm" style={{ boxShadow: 'var(--admin-shadow-neu-inset)', color: 'var(--admin-text-secondary)' }}>
              <FiPause />
          ...
          <div className="flex flex-row items-center gap-1.5 w-full md:flex-1 overflow-x-auto no-scrollbar pb-1 md:pb-0" style={{ WebkitOverflowScrolling: 'touch' }}>
            {uniqueSlugs.map((cat) => (
              <button
                ...
                style={{ borderColor: selectedSlug === cat ? 'transparent' : 'var(--admin-border)' }}
                data-cursor="hover"
              >
                {cat}
              </button>
        ...
        <div className="overflow-x-auto w-full [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-3">
              ...
              <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--admin-text-tertiary)]">Loading Services...</span>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 px-6 text-center max-w-md mx-auto">
              ...
              <h3 className="font-display font-extrabold uppercase tracking-wider text-sm mb-2" style={{ color: 'var(--admin-text-primary)' }}>
                Your Creative services is Empty
              </h3>
              <p className="font-body text-xs text-[var(--admin-text-tertiary)] leading-relaxed mb-6">
                Start listing your premium design and technology packages. Added items will instantly publish to the client-facing services page.
              </p>
              <button
                onClick={() => openFormModal(null)}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[var(--admin-bg)] border-b" style={{ borderColor: 'var(--admin-border)' }}>
                  ...
                  <th className="p-4 font-mono text-[9px] uppercase tracking-wider text-[var(--admin-text-secondary)]">Slug</th>
                  <th className="p-4 font-mono text-[9px] uppercase tracking-wider text-[var(--admin-text-secondary)]">Tagline</th>
              ...
              <tbody>
                {filteredItems.map((item) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3.5 max-w-sm">
                        ...
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            <span className="font-body font-bold text-xs leading-snug">{item.title}</span>
                            {item.isActive === false ? (
                    ...
                    {/* Slug Column */}
                    <td className="p-4">
                      <span className="px-2.5 py-1 rounded-full text-[9px] font-mono uppercase tracking-wider border font-bold" style={{ borderColor: 'var(--admin-border)', color: 'var(--admin-text-secondary)' }}>
                        {item.slug}
                      </span>
                    </td>
            ...
            >
              ...
              <form onSubmit={handleSaveItem} className="flex flex-col flex-1 overflow-hidden">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  />
                  {formErrors.title && (
                    <span className="text-[10px] text-red-500 font-bold tracking-wide mt-0.5">{formErrors.title}</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    />
                    {formErrors.slug && (
                      <span className="text-[10px] text-red-500 font-bold tracking-wide mt-0.5">{formErrors.slug}</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      type="text"
                      placeholder="e.g. Websites that win business."
                    ...
                    />
                    {formErrors.tagline && (
                      <span className="text-[10px] text-red-500 font-bold tracking-wide mt-0.5">{formErrors.tagline}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 border-t pt-4 mt-5" style={{ borderColor: 'var(--admin-border)' }}>
                  ...
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="px-5 py-2.5 rounded-lg text-xs font-body font-bold uppercase tracking-wider bg-[var(--color-orange)] hover:bg-[var(--color-orange-light)] text-white shadow transition-colors cursor-pointer flex items-center justify-center min-w-[140px] disabled:opacity-70 disabled:cursor-not-allowed"
                    data-cursor={isSaving ? "default" : "hover"}
                  >
                    {isSaving ? (
